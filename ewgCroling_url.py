...
name=(name.iloc[:20,[0]]).values.tolist()
keys = []
for i in name:
    keys.append(i[0])
print(keys)
...
def croling(data, count_dict, processnum):
    ...
    while count_dict['count'] < len(keys):
        ...
        try:
        #if(type(key)==str or math.isnan(key)==False): 
            driver.get(url=URL)
            #//*[@id="__next"]/div/div/div[1]/div[2]/div/div[1]/div/div/div[2]/div/div[1]/i
            search_box = driver.find_element('xpath', '//*[@id="open-search"]')
            search_box.click()
            search_box = driver.find_elements('xpath', '//*[@id="search"]')
            search_box = search_box[2]
            print(key)
            search_box.send_keys(key)
            time.sleep(0.3)
            search_list = driver.find_elements('xpath', '//*[@id="eac-container-search"]/ul')
            search_list = search_list[2]
            print('style 추출전')
